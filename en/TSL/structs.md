...
Structs let you combine multiple related values of different types into a single custom data structure.
...
**Syntax:**
- `destruct StructName` starts a struct definition
- Each field on a new line with `fieldName -> type`
- Access fields with `variable.fieldName`
- Set fields with `variable.fieldName = value`
...
## Practical Example: Game Character
...
### Forgetting the Arrow in Field Type
```tsl
destruct Point
...
Define structs BEFORE using them:
...
### Modifying Struct Through Value vs. Pointer
```tsl
destruct Point
    x -> int
    y -> int
...
1. **Logical Grouping**: Group related fields together in one struct
2. **Meaningful Names**: Use clear names for structs and fields
3. **Simple Structs**: Avoid overly complex nested structures initially
4. **Use Functions**: Extract struct operations into reusable functions
5. **Pass by Reference**: For performance, use pointers when modifying structs
6. **Document Fields**: Add comments explaining complex fields
7. **Keep Type Consistency**: All instances of a struct use the same field types

## Next Steps

- Combine structs with **[Functions](functions.md)** for data processing
- Use **[Arrays](lists.md)** of structs for collections
- Explore **[Pointers](pointers.md)** to modify structs efficiently
- Try **[Enums](enums.md)** for fixed sets of values
